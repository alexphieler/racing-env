...
/**
 * Complete, typed configuration for VehicleModel4Wheel.
 *
 * This deliberately belongs to the concrete model instead of IVehicleModel:
 * the parameters and their meaning are specific to the 4-wheel dynamics.
 */
struct VehicleModel4WheelParameters
{
    double lf = 0.78;
    double lr = 0.72;
    double sf = 1.15;
    ...
    double steeringW0 = 35.0;
    double steeringD = 0.6;
    double steeringMax = 1.834;
    double steeringMaxRate = 5.0;

    void validate() const
    {
        for (const double value : {
                 lf, lr, sf, sr,
                 Blat, Clat, Dlat, Elat, relaxationLengthLat,
                 Blon, Clon, Dlon, Elon, relaxationLengthLon,
                 cla, cda, aeroArea, m, hc, Izz, wheelRadius, gearRatio,
                 innerSteeringRatio, outerSteeringRatio, nominalVoltageTS,
                 powerGroundForce, powertrainEfficiency, wspdControlKp,
                 wspdControlKi, steeringW0, steeringD, steeringMax,
                 steeringMaxRate })
        {
            if (!std::isfinite(value))
            {
                throw std::invalid_argument("VehicleModel4Wheel parameters must be finite");
            }
        }

        if (lf <= 0.0 || lr <= 0.0 || sf <= 0.0 || sr <= 0.0
            || Dlat <= 0.0 || Dlon <= 0.0
            || relaxationLengthLat <= 0.0 || relaxationLengthLon <= 0.0
            || cda < 0.0 || aeroArea <= 0.0 || m <= 0.0 || hc < 0.0
            || Izz <= 0.0 || wheelRadius <= 0.0 || gearRatio <= 0.0
            || innerSteeringRatio <= 0.0 || outerSteeringRatio <= 0.0
            || nominalVoltageTS <= 0.0 || powerGroundForce < 0.0
            || powertrainEfficiency <= 0.0 || powertrainEfficiency > 1.0
            || wspdControlKp < 0.0 || wspdControlKi < 0.0 || steeringW0 <= 0.0
            || steeringD < 0.0 || steeringMax <= 0.0 || steeringMaxRate <= 0.0)
        {
            throw std::invalid_argument("VehicleModel4Wheel parameters are outside their physical bounds");
        }
    }
};
...
public:
    ...
    {
        VehicleModel4WheelParameters parameters = getParameters();
        auto configModel = config["simple_bicycle_model"];
        configModel["kinematics"].getElement<double>(&parameters.lf, "lf");
        configModel["kinematics"].getElement<double>(&parameters.lr, "lr");
        configModel["kinematics"].getElement<double>(&parameters.sf, "sf");
        configModel["kinematics"].getElement<double>(&parameters.sr, "sr");

        configModel["tire"].getElement<double>(&parameters.Blat, "Blat");
        configModel["tire"].getElement<double>(&parameters.Clat, "Clat");
        configModel["tire"].getElement<double>(&parameters.Dlat, "Dlat");
        configModel["tire"].getElement<double>(&parameters.Elat, "Elat");

        configModel["aero"].getElement<double>(&parameters.cla, "cla");
        configModel["aero"].getElement<double>(&parameters.cda, "cda");
        configModel["aero"].getElement<double>(&parameters.aeroArea, "aeroArea");

        configModel.getElement<double>(&parameters.m, "m");
        configModel.getElement<double>(&parameters.Izz, "Izz");
        configModel.getElement<double>(&parameters.wheelRadius, "wheelRadius");
        configModel.getElement<double>(&parameters.gearRatio, "gearRatio");
        configModel.getElement<double>(&parameters.innerSteeringRatio, "innerSteeringRatio");
        configModel.getElement<double>(&parameters.outerSteeringRatio, "outerSteeringRatio");
        configModel.getElement<double>(&parameters.nominalVoltageTS, "nominalVoltageTS");
        configModel.getElement<double>(&parameters.powerGroundForce, "powerGroundForce");
        configModel.getElement<double>(&parameters.powertrainEfficiency, "powertrainEfficiency");

        configure(parameters);
        return true;
    }

    VehicleModel4WheelParameters getParameters() const
    {
        VehicleModel4WheelParameters parameters;
        parameters.lf = lf;
        parameters.lr = lr;
        parameters.sf = sf;
        parameters.sr = sr;
        parameters.Blat = Blat;
        parameters.Clat = Clat;
        parameters.Dlat = Dlat;
        parameters.Elat = Elat;
        parameters.relaxationLengthLat = relaxationLengthLat;
        parameters.Blon = Blon;
        parameters.Clon = Clon;
        parameters.Dlon = Dlon;
        parameters.Elon = Elon;
        parameters.relaxationLengthLon = relaxationLengthLon;
        parameters.cla = cla;
        parameters.cda = cda;
        parameters.aeroArea = aeroArea;
        parameters.m = m;
        parameters.hc = hc;
        parameters.Izz = Izz;
        parameters.wheelRadius = wheelRadius;
        parameters.gearRatio = gearRatio;
        parameters.innerSteeringRatio = innerSteeringRatio;
        parameters.outerSteeringRatio = outerSteeringRatio;
        parameters.nominalVoltageTS = nominalVoltageTS;
        parameters.powerGroundForce = powerGroundForce;
        parameters.powertrainEfficiency = powertrainEfficiency;
        parameters.wspdControlKp = wspdControlKp;
        parameters.wspdControlKi = wspdControlKi;
        parameters.steeringW0 = steering_w0;
        parameters.steeringD = steering_d;
        parameters.steeringMax = steering_max;
        parameters.steeringMaxRate = steering_max_rate;
        return parameters;
    }

    void configure(const VehicleModel4WheelParameters& parameters)
    {
        parameters.validate();
        lf = parameters.lf;
        lr = parameters.lr;
        sf = parameters.sf;
        sr = parameters.sr;
        Blat = parameters.Blat;
        Clat = parameters.Clat;
        Dlat = parameters.Dlat;
        Elat = parameters.Elat;
        relaxationLengthLat = parameters.relaxationLengthLat;
        Blon = parameters.Blon;
        Clon = parameters.Clon;
        Dlon = parameters.Dlon;
        Elon = parameters.Elon;
        relaxationLengthLon = parameters.relaxationLengthLon;
        cla = parameters.cla;
        cda = parameters.cda;
        aeroArea = parameters.aeroArea;
        m = parameters.m;
        hc = parameters.hc;
        Izz = parameters.Izz;
        wheelRadius = parameters.wheelRadius;
        gearRatio = parameters.gearRatio;
        innerSteeringRatio = parameters.innerSteeringRatio;
        outerSteeringRatio = parameters.outerSteeringRatio;
        nominalVoltageTS = parameters.nominalVoltageTS;
        powerGroundForce = parameters.powerGroundForce;
        powertrainEfficiency = parameters.powertrainEfficiency;
        wspdControlKp = parameters.wspdControlKp;
        wspdControlKi = parameters.wspdControlKi;
        steering_w0 = parameters.steeringW0;
        steering_d = parameters.steeringD;
        steering_max = parameters.steeringMax;
        steering_max_rate = parameters.steeringMaxRate;
    }
