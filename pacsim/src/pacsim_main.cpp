...
std::vector<std::shared_ptr<CameraImageSensor>> cameraSensors;
std::vector<rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr> cameraImagePublishers;
std::vector<rclcpp::Publisher<sensor_msgs::msg::CameraInfo>::SharedPtr> cameraInfoPublishers;
std::shared_ptr<VulkanCameraSim> cameraSim;
VulkanCameraSim::RuntimeOptions cameraRuntimeOptions;
...
std::string car_xacro_path;
std::string left_cone_asset_path;
std::string right_cone_asset_path;
std::string ground_plane_asset_path;
std::string skybox_asset_path;
std::string discipline;
std::vector<std::string> jointNames
    = { "RR_steer","RL_steer","FL_steer", "FR_steer", "FL_rotate", "FR_rotate", "RR_rotate", "RL_rotate", "steering" };
double realtimeRatio = 1.0;
double maxRelativeRealtimeRatio = 1.2;
...
int threadMainLoopFunc(std::shared_ptr<rclcpp::Node> node)
{
    ...
    if (!cameraSensors.empty() && !cameraSim)
    {
        cameraSim = std::make_shared<VulkanCameraSim>(
            0, 0, cameraRuntimeOptions.modelRoot, cameraRuntimeOptions.cameraConfigPath,
            cameraRuntimeOptions.carXacroPath);
        cameraSim->setAssetPaths(cameraRuntimeOptions.leftConeAssetPath,
            cameraRuntimeOptions.rightConeAssetPath,
            cameraRuntimeOptions.groundPlaneAssetPath, cameraRuntimeOptions.skyboxAssetPath);
        cameraSim->setShadowsEnabled(cameraRuntimeOptions.shadowsEnabled);
    }

    if (cameraSim)
    {
        cameraSim->setTrackAndCones(lms);

        if (cameraSim->cameraCount() != cameraSensors.size())
        {
            throw std::runtime_error("Camera scheduler mismatch: simulator camera count and sensor wrappers differ");
        }
    }
    ...
    while (rclcpp::ok() && !(finish))
    {
        ...
        Wheels wheelOrientations = model->getWheelOrientations();

        bool anyCameraDue = false;
        for (const auto& cameraSensor : cameraSensors)
        {
            if (cameraSensor->isEnabled() && cameraSensor->sampleReady(simTime))
            {
                anyCameraDue = true;
                break;
            }
        ...
        if (anyCameraDue)
        {
            auto renderedImages = cameraSim->render(t, rEulerAngles, static_cast<float>(steeringWheelCurr), wheelOrientations);
            const auto cameraIntrinsics = cameraSim->cameraIntrinsics();

            const int imageWidth = cameraSim->width();
            const int imageHeight = cameraSim->height();
            const size_t cameraCount = std::min(renderedImages.size(), cameraSensors.size());
            for (size_t i = 0; i < cameraCount; ++i)
            {
                CameraImageFrame frame;
                frame.data = std::move(renderedImages[i]);
                frame.width = imageWidth;
                frame.height = imageHeight;
                if (i < cameraIntrinsics.size())
                {
                    frame.fx = static_cast<double>(cameraIntrinsics[i].fx);
                    frame.fy = static_cast<double>(cameraIntrinsics[i].fy);
                    frame.cx = static_cast<double>(cameraIntrinsics[i].cx);
                    frame.cy = static_cast<double>(cameraIntrinsics[i].cy);
                }
                frame.encoding = "bgr8";
                frame.timestamp = simTime;
                frame.frame = cameraSensors[i]->getFrameId();
                cameraSensors[i]->RunTick(frame, simTime);
            }
        }

        for (size_t i = 0; i < cameraSensors.size(); ++i)
        {
            if (cameraSensors[i]->availableDeadTime(simTime))
            {
                const auto imageFrame = cameraSensors[i]->getOldest();
                cameraImagePublishers[i]->publish(createRosImageMsg(imageFrame));
                cameraInfoPublishers[i]->publish(createRosCameraInfoMsg(imageFrame));
            }
        }
        ...
        std::vector<double> jointMsg = { steeringCurr.RR,steeringCurr.RL,steeringCurr.FL, steeringCurr.FR, 
            wheelOrientations.FL, wheelOrientations.FR, wheelOrientations.RR, wheelOrientations.RL, -steeringWheelCurr };
        ...
        const auto minLoopTime = loopStartTime + std::chrono::microseconds(
            (int)((timestep / (realtimeRatio * maxRelativeRealtimeRatio)) * 1000000.0));
        std::this_thread::sleep_until(std::max(nextLoopTime, minLoopTime));
    }
    Report report;
...
void getRos2Params(rclcpp::Node::SharedPtr& node)
{
    ...
    params.push_back({ "cameras_config_path", &camera_config_path });
    params.push_back({ "car_xacro_path", &car_xacro_path });
    params.push_back({ "left_cone_asset_path", &left_cone_asset_path });
    params.push_back({ "right_cone_asset_path", &right_cone_asset_path });
    params.push_back({ "ground_plane_asset_path", &ground_plane_asset_path });
    params.push_back({ "skybox_asset_path", &skybox_asset_path });
    ...
    node->declare_parameter("max_relative_realtime_ratio", rclcpp::PARAMETER_DOUBLE);
    node->get_parameter("max_relative_realtime_ratio", maxRelativeRealtimeRatio);
}
...
void initSensors()
{
    ...
    cameraRuntimeOptions = VulkanCameraSim::RuntimeOptions {};
    cameraRuntimeOptions.cameraConfigPath = camera_config_path;
    cameraRuntimeOptions.carXacroPath = car_xacro_path;
    cameraRuntimeOptions.leftConeAssetPath = left_cone_asset_path;
    cameraRuntimeOptions.rightConeAssetPath = right_cone_asset_path;
    cameraRuntimeOptions.groundPlaneAssetPath = ground_plane_asset_path;
    cameraRuntimeOptions.skyboxAssetPath = skybox_asset_path;

    cameraSensors.clear();
    cameraImagePublishers.clear();
    cameraInfoPublishers.clear();
    cameraSim.reset();

    if (!cameraRuntimeOptions.cameraConfigPath.empty())
    {
        auto tempSim = std::make_shared<VulkanCameraSim>(
            0, 0, cameraRuntimeOptions.modelRoot, cameraRuntimeOptions.cameraConfigPath,
            cameraRuntimeOptions.carXacroPath);
        tempSim->setAssetPaths(cameraRuntimeOptions.leftConeAssetPath,
            cameraRuntimeOptions.rightConeAssetPath,
            cameraRuntimeOptions.groundPlaneAssetPath, cameraRuntimeOptions.skyboxAssetPath);

        const auto names = tempSim->cameraNames();
        const auto enabled = tempSim->cameraEnabledFlags();
        const auto rates = tempSim->cameraRatesHz();
        const auto delays = tempSim->cameraDelayMeans();
        const auto positions = tempSim->cameraLocalPositions();
        const auto orientations = tempSim->cameraLocalOrientations();

        cameraSensors.reserve(names.size());
        for (size_t i = 0; i < names.size(); ++i)
        {
            auto cameraSensor = std::make_shared<CameraImageSensor>();
            const double rate = (i < rates.size()) ? static_cast<double>(rates[i]) : 10.0;
            const double delay = (i < delays.size()) ? static_cast<double>(delays[i]) : 0.0;
            const bool isEnabled = (i < enabled.size()) ? enabled[i] : true;
            const Eigen::Vector3d position = (i < positions.size()) ? positions[i] : Eigen::Vector3d::Zero();
            const Eigen::Vector3d orientation = (i < orientations.size()) ? orientations[i] : Eigen::Vector3d::Zero();
            cameraSensor->configure(names[i], names[i], rate, delay, isEnabled, position, orientation);
            cameraSensors.push_back(cameraSensor);
        }

        logger->logInfo("Initialized camera simulator with " + std::to_string(cameraSensors.size()) + " cameras");
    }
}
...
MainConfig fillMainConfig(std::string path)
{
    ...
    config.getElement<bool>(&ret.camera_tf2_optical, "camera_tf2_optical");
...
void handleTf2StaticTransforms()
{
    if (mainConfig.broadcast_sensors_tf2)
    {
        ...
        for (auto sensor : cameraSensors)
        {
            ...
            tf2::Quaternion q;
            q.setRPY(orientation.x(), orientation.y(), orientation.z());
            if (mainConfig.camera_tf2_optical)
            {
                // Convert ROS camera frame (x forward, y left, z up) to optical frame
                // (z forward, x right, y down).
                tf2::Quaternion qOptical;
                qOptical.setRPY(-1.57079632679, 0.0, -1.57079632679);
                q = q * qOptical;
            }
            t.transform.rotation.x = q.x();
            t.transform.rotation.y = q.y();
...
int main(int argc, char** argv)
{
    ...
    for (auto& cameraSensor : cameraSensors)
    {
        const std::string topicPrefix = "/pacsim/camera/" + cameraSensor->getName();
        auto imagePub = node->create_publisher<sensor_msgs::msg::Image>(topicPrefix + "/image_raw", 1);
        auto cameraInfoPub = node->create_publisher<sensor_msgs::msg::CameraInfo>(topicPrefix + "/camera_info", 1);
        cameraImagePublishers.push_back(imagePub);
        cameraInfoPublishers.push_back(cameraInfoPub);
    }
    if (cameraSensors.size() != cameraImagePublishers.size()
        || cameraSensors.size() != cameraInfoPublishers.size())
    {
        throw std::runtime_error("Camera publisher mismatch: sensors, image publishers, and camera info publishers must align");
    }
